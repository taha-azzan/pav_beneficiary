...
frappe.ui.form.on('Beneficiary Modification', {
...
frappe.ui.form.on(cur_frm.doctype, {
	...
	onload: function(frm){
		if(frm.doc.__islocal){
			if(frm.doctype == "Beneficiary Modification"){
				frm.doc.beneficiary_modification_details = [];
	...
	beneficiary: function(frm) {
		frm.add_fetch("beneficiary", "company", "company");
	},
	refresh: function(frm) {
		var table;
		if(frm.doctype == "Beneficiary Modification"){
			table = "beneficiary_modification_details";
		}else if (frm.doctype == "Employee Transfer") {
		...
		cur_frm.fields_dict[table].grid.add_custom_button(__('Add Row'), () => {
			if(!frm.doc.beneficiary){
				frappe.msgprint(__("Please select Beneficiary"));
				return;
			}
			frappe.call({
